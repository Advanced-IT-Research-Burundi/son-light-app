...
<body>
    ...
    <h2 style="color:red;">FACTURE  du {{ $order->created_at->format('d/m/Y') }}</h2>
    ...
    <div style=" font-size: 14px;" >
        ...
        <h3>B. Le Client</h3>
        <p>
            <strong>Nom et prénom ou raison sociale :</strong> {{ $order->client->name }}<br>
            <strong>NIF :</strong> {{ $order->client->nif ?? '_________' }}<br>
            <strong>Résidence à :</strong> {{ $order->client->address ?? 'BUJA' }}<br>
            <strong>Assujetti à la TVA :</strong> Oui [ {{ $order->client->assujeti?'X':' ' }}] Non [{{ $order->client->assujeti?' ':'X' }}]
    ...
    <table class="table2">
        ...
        <tfoot>
            <tr>
                ...
                <td>{{ number_format($order->detailOrders->sum('total_price'), 2) }}</td>
                <td>{{ $order->entreprise->assujeti?number_format($order->detailOrders->sum('total_price') * $order->tva / 100, 0):'' }}</td>
                <td>{{ $order->entreprise->assujeti?number_format($order->detailOrders->sum('total_price') * (1 + $order->tva / 100), 0):'' }}</td>
            </tr>
        </tfoot>
    </table>
    <div>
